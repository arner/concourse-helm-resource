...
Alternatively you can set the following parameters under the `ibmcloud` key:
* `api_url`: *Required.* URL to the API endpoint in your region
* `api_key`: *Required.* API key for your account
* `organization`: *Required.* Your IBM Cloud organization
* `space`: *Required.* The space where your Kubernetes cluster lives
* `cluster_name`: *Required.* The name of your Kubernetes cluster
...
- name: myapp-helm
  ...
  source:
    ibmcloud:
      api_url: https://api.eu-de.bluemix.net
      api_key: cOpiL9m6GGEdl0Jkld_e0zFW
      organization: username@example.com
      space: myspace
